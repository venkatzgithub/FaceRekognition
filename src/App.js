...
class App extends Component {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch('https://guarded-gorge-19251.herokuapp.com/imageurl', {
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch('https://guarded-gorge-19251.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
  ...
  render() {
    ...
    return (
      ...
      <Rank  name={this.state.user.name}
                entries={this.state.user.entries}></Rank>
      <ImageForm onInputChange={this.onInputChange} onButtonSubmit={this.onButtonSubmit}  ></ImageForm>
      <FaceRecognition box={box} imageUrl={imageUrl}></FaceRecognition> </div>:
      (
        route === 'signin'
